...
|---|---|---|
| `tools/repo-drift-scanner/` | Runnable Python utility for stale-truth detection plus deterministic repository contracts, source authority, structured state, lifecycle, current surfaces, output ownership, append-only history, review signals, source-coverage accounting, evidence-domain exclusions, machine-readable output, cross-model handoff, and tests | Published / runnable — v0.3.2 |
...
Current artifacts:
- `tools/repo-drift-scanner/` — Repo Drift Scanner v0.3.2, a deterministic stale-truth and repository-contract validator with source-coverage visibility, evidence-domain boundaries, self-reference hardening, fictional examples, and `AI_HANDOFF.md` for cross-model continuity
