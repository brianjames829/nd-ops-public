...
from drift_core import ConfigError, DEFAULT_EXCLUDES, PatternSpec, SLUG, classify_authority, search_pattern
from drift_scope import ScanBoundary

CONTRACT_TYPES = {
    'authority', 'structured_assertion', 'lifecycle',
    'current_surface', 'output_ownership', 'append_only',
}
...
def load_contracts(path: Path) -> list[Contract]:
    ...
    for index, raw in enumerate(raw_contracts):
        ...
        if contract_type not in CONTRACT_TYPES:
            raise ConfigError(f"{where}.type must be one of: {', '.join(sorted(CONTRACT_TYPES))}")
        ...
        if contract_type == 'authority':
            _reject_unknown(item, common | {'paths', 'must_be', 'require_match'}, where)
            ...
            _reject_unknown(item, common | {'path', 'pointer', 'op', 'value'}, where)
            ...
            if op != 'exists' and 'value' not in item:
                raise ConfigError(f'{where}.value is required for op {op}')
            spec = {
                'path': _expect_string(item.get('path'), f'{where}.path'),
            ...
            _reject_unknown(item, common | {'path', 'invariants', 'record_id_field'}, where)
            inv_raw = _expect_list(item.get('invariants'), f'{where}.invariants')
            if not inv_raw:
                raise ConfigError(f'{where}.invariants must contain at least one invariant')
            invariants = []
            for i, raw_inv in enumerate(inv_raw):
                iw = f'{where}.invariants[{i}]'
                inv = _expect_dict(raw_inv, iw)
                _reject_unknown(inv, {'when', 'require', 'message'}, iw)
                msg = inv.get('message')
                if msg is not None:
                    msg = _expect_string(msg, f'{iw}.message')
                invariants.append({
            ...
            spec = {
                ...
                'record_id_field': _expect_string(item.get('record_id_field', 'id'), f'{where}.record_id_field'),
            }
        elif contract_type == 'current_surface':
            _reject_unknown(item, common | {
                'path', 'must_be_authority', 'required_patterns', 'forbidden_patterns', 'review_patterns'
            }, where)
            ...
            for key in ('required_patterns', 'forbidden_patterns', 'review_patterns'):
                values = _expect_list(item.get(key, []), f'{where}.{key}')
            ...
            spec = {
                ...
                'reference': _expect_string(item.get('reference', output), f'{where}.reference'),
                'require_owner_reference': bool(item.get('require_owner_reference', True)),
            }
        else:
            _reject_unknown(item, common | {'path'}, where)
            spec = {'path': _expect_string(item.get('path'), f'{where}.path')}

        contracts.append(Contract(contract_id, contract_type, description, level, source, reason, spec))
    return contracts


def _iter_files(root: Path, boundary: ScanBoundary | None = None) -> Iterable[Path]:
    for path in sorted(root.rglob('*')):
        ...
        if any(part in DEFAULT_EXCLUDES for part in path.relative_to(root).parts):
            continue
        if boundary is not None and boundary.excludes(rel):
            continue
        yield path
...
def _finding(contract: Contract, path: str, message: str, *, details=None, level=None) -> ContractFinding:
    return ContractFinding(
        contract.contract_id, contract.contract_type, contract.description,
        level or contract.level, path, message, details or {}, contract.source, contract.reason
    )
...
def _evaluate_authority(root: Path, contract: Contract, authority_rules, boundary) -> list[ContractFinding]:
    findings = []
    matched = []
    for path in _iter_files(root, boundary):
        rel = path.relative_to(root).as_posix()
    ...
    for line_no, line in enumerate(lines, 1):
        ...
        for inv in contract.spec['invariants']:
            if _predicate(record, inv['when']) and not _predicate(record, inv['require']):
                findings.append(_finding(
                    contract, rel,
                    inv['message'] or 'lifecycle invariant failed',
                    details={
                        'line': line_no,
                        'record_id': record_id,
                        'when': inv['when'],
...
def _evaluate_current_surface(root: Path, contract: Contract, authority_rules) -> list[ContractFinding]:
    ...
    findings = []
    expected = contract.spec.get('must_be_authority')
    if expected is not None:
        actual = classify_authority(rel, authority_rules)
        if actual != expected:
            findings.append(_finding(
                contract, rel,
                f'current surface authority is {actual!r}; expected {expected!r}',
                details={'actual': actual, 'expected': expected},
            ))
    lines = text.splitlines()
    for pattern in contract.spec.get('required_patterns', []):
        if not any(search_pattern(pattern, line) for line in lines):
            findings.append(_finding(
                contract, rel,
...
def _evaluate_output_ownership(root: Path, contract: Contract, boundary) -> list[ContractFinding]:
    findings = []
    owners_seen = []
...
def _baseline_text(root: Path, baseline: str, rel: str) -> tuple[bool, str]:
    ...
    if exists.returncode != 0:
        ...
        if ref_check.returncode != 0:
            raise ContractEvaluationError(ref_check.stderr.strip() or f'cannot resolve baseline {baseline}')
        return False, ''
    shown = _run_git(root, 'show', f'{baseline}:{rel}')
    if shown.returncode != 0:
        raise ContractEvaluationError(shown.stderr.strip() or f'cannot read {rel} from {baseline}')
    return True, shown.stdout


def _evaluate_append_only(root: Path, contract: Contract, baseline: str | None) -> list[ContractFinding]:
    rel = contract.spec['path']
    if baseline is None:
        return [_finding(
            contract, rel,
...
def _glob_magic(value: str) -> bool:
    return any(ch in value for ch in '*?[')


def validate_contract_boundary(contracts: list[Contract], boundary: ScanBoundary | None) -> None:
    if boundary is None or not boundary.rules:
        return
    for contract in contracts:
        targets: list[str] = []
        if contract.contract_type in {'structured_assertion', 'lifecycle', 'current_surface', 'append_only'}:
            targets.append(contract.spec['path'])
        elif contract.contract_type == 'output_ownership':
            targets.append(contract.spec['output'])
        elif contract.contract_type == 'authority':
            targets.extend(p for p in contract.spec['paths'] if not _glob_magic(p))
        for target in targets:
            rule = boundary.match(target)
            if rule is not None:
                raise ConfigError(
                    f'contract {contract.contract_id!r} targets excluded path {target!r} '
                    f'({rule.source}: {rule.pattern})'
                )
...
def evaluate_contracts(
    ...
    *,
    ...
    boundary: ScanBoundary | None = None,
) -> ContractResult:
    validate_contract_boundary(contracts, boundary)
    findings: list[ContractFinding] = []
    for contract in contracts:
        if contract.contract_type == 'authority':
            findings.extend(_evaluate_authority(root, contract, authority_rules, boundary))
            ...
            findings.extend(_evaluate_output_ownership(root, contract, boundary))
